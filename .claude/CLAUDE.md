...
- security：集成 Spring Security 6 + JWT，实现细粒度 RBAC 权限控制
...
### log 模块详解
满足操作留痕的日志模块。
日志防篡改：操作日志和认证日志都使用 SM3 哈希链保护，算法为 `hash = SM3(id + "|" + prevHash)`。
任意一条日志被篡改会导致后续所有日志哈希验证失败。
全链路追踪：通过 traceId 将一次请求从进入系统到返回响应的所有日志串联起来，便于问题排查和操作回溯。
日志通过注解声明记录。在需要记录的方法上添加 @Log 注解，框架自动记录操作人、操作时间、操作内容、操作结果等信息。
使用方式：
