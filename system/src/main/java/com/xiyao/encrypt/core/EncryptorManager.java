...
public class EncryptorManager {
     ...
     * 获取类的加密字段缓存
     * <p>
     * 缓存带有 @EncryptField 注解且类型为 String 的字段。
     *
     * @param clazz 待检查的类
     * @return 加密字段集合
     */
    ...
    /**
     * 扫描获取类中所有带 @EncryptField 注解的字段
     * <p>
     * 遍历类及其父类的所有声明字段，
     * 筛选出标注了 @EncryptField 且类型为 String 的字段。
     *
     * @param clazz 待扫描的类
     * @return 加密字段集合
     */
    private Set<Field> getEncryptField(Class<?> clazz) {
        ...
        while (current != null) {
            for (Field field : current.getDeclaredFields()) {
                // 检查是否有 @EncryptField 注解，且字段类型必须是 String
