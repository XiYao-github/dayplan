 ...
 *     <li>入参拦截器遍历参数对象，对带有 @EncryptField 注解的字段进行加密</li>
 *     <li>出参拦截器遍历结果集，对带有 @EncryptField 注解的字段进行解密</li>
...
public class EncryptorDataConfig {
     ...
     * 创建入参加密拦截器
     * <p>
     * 拦截 MyBatis 入参，在数据写入数据库前对带有 @EncryptField 注解的字段进行加密。
     *
     ...
     * 创建出参解密拦截器
     * <p>
     * 拦截 MyBatis 出参，在数据查询出数据库后对带有 @EncryptField 注解的字段进行解密。
     *
     * @param encryptorManager 加密管理器
     * @param properties       配置属性
